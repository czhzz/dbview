...
- [ ] 修复 `connection-store.ts` 中 `getById()` SELECT 列表补上 `read_only` 字段

### P1-1 datatable-pagination（2h）
- [ ] DataTable 编辑模式下分页/排序/刷新前检查 `pendingChanges`
- [ ] 有未保存更改时弹出确认对话框（保存并继续 / 丢弃并继续 / 取消）
- [ ] 无更改时直接切换

### P1-2 column-type-filter（2h）
- [ ] 将 `COLUMN_TYPES` 重构为 `COLUMN_TYPES_BY_DB` 映射表
- [ ] ColumnDialog 根据 `dbType` prop 选择对应类型列表
- [ ] 验证四种数据库的类型列表完整性

### P1-3 ddl-viewer-dialect（1h）
- [ ] DDLViewer 新增 `dbType` prop
- [ ] 根据 dbType 选择 CodeMirror SQL 方言
- [ ] StructurePage 传入 dbType

### P1-6 sql-editor-rebuild（2h）
- [ ] EditorView destroy 前保存 `doc.toString()`
- [ ] 重建时传入保存的内容作为 `doc`
- [ ] 验证主题切换和数据库切换场景
