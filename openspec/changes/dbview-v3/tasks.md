...
- [x] 4.1.1 在 main 进程创建 `getErDiagramData` 查询（获取表/列/PK/FK 元数据）
- [x] 4.1.2 创建 ERDiagram 组件，集成 React Flow + dagre 自动布局
- [x] 4.1.3 实现表实体节点渲染（表名、PK 图标、列类型）
- [x] 4.1.4 实现关系连线（带基数标记 1:1/1:N/N:M）
- [x] 4.1.5 实现缩放/平移/双击聚焦
- [x] 4.1.6 实现点击表节点跳转到结构视图
- [x] 4.1.7 实现搜索过滤表、显示相关表、显示全部
- [x] 4.1.8 实现布局持久化（保存/恢复节点位置）

### 4.2 连接健康监测

- [x] 4.2.1 ConnectionManager 增加心跳定时器管理
- [x] 4.2.2 实现 `SELECT 1` 心跳查询（可配置间隔）
- [x] 4.2.3 实现自动重连逻辑（3 次重试，10 秒间隔）
- [x] 4.2.4 实现连接状态指示器（绿/黄/红/灰圆点）
- [x] 4.2.5 实现状态 tooltip 显示详情
- [x] 4.2.6 实现断连通知提示
...
- [x] 5.1.1 创建 `DiffService`（StructureComparer 组件）
- [x] 5.1.2 实现表级对比（新增/缺失/修改/相同）
- [x] 5.1.3 实现列级对比（类型/可空/默认值差异）
- [x] 5.1.4 实现索引级对比
- [x] 5.1.5 创建 DiffViewer 组件（结构对比结果展示）
- [x] 5.1.6 实现 `MigrationGenerator`（从差异生成 ALTER DDL）
- [x] 5.1.7 实现迁移 SQL 预览和执行
- [x] 5.1.8 实现 Dry Run 模式
- [x] 5.1.9 实现数据对比（按主键逐行对比）

### 5.2 数据导入

- [x] 5.2.1 创建 `ImportService`（CsvParser 组件）
- [x] 5.2.2 实现 CSV 解析（编码检测、类型推断）
- [x] 5.2.3 实现 JSON 解析（数组对象、嵌套展平）
- [x] 5.2.4 实现 Excel 解析（多 sheet 选择）
- [x] 5.2.5 创建 DataImport 组件（文件选择、预览、字段映射）
- [x] 5.2.6 实现字段映射 UI（拖拽匹配 CSV 列 → 表列）
- [x] 5.2.7 实现类型不匹配警告
- [x] 5.2.8 实现分批导入（可配置批次大小，进度反馈）
- [x] 5.2.9 实现导入错误处理（跳过/重试/中止）
- [x] 5.2.10 实现从导入数据创建新表
