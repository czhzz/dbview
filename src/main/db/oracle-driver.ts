...
export class OracleDriver implements DatabaseDriver {
  ...
  /**
   * Get a dedicated connection for executeQuery (needs conn.break() for cancellation).
   * All other methods use pool.execute() which borrows/releases automatically.
   */
  private async getConnection(): Promise<Connection> {
    return this.getPool().getConnection()
  }

  // ---- Metadata methods: use pool.execute() for automatic connection management ----
  ...
  async getViews(schema?: string): Promise<ViewInfo[]> {
    const schemaName = (schema || this.config?.username || '').toUpperCase()
    const result = await this.getPool().execute<{ view_name: string }>(
  ...
  async getDDL(table: string, schema?: string): Promise<string> {
    ...
    // Get columns (uses pool.execute internally)
    ...
    // Add comments
    const colRows = await this.getPool().execute<{ column_name: string; comments: string | null }>(
  ...
  async getRoutineDefinition(name: string, _type: 'PROCEDURE' | 'FUNCTION', schema?: string): Promise<string> {
    const schemaName = (schema || this.config?.username || '').toUpperCase()
    const result = await this.getPool().execute<{ text: string }>(
  ...
  // ---- queryPage: uses pool.execute() for automatic connection management ----
